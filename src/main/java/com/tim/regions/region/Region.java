...
public class Region {
    ...
    public Location getCenter() {
        return new Location(this.world, (double)(this.xMax - this.xMin) / 2.0 + (double)this.xMin, (double)(this.yMax - this.yMin) / 2.0 + (double)this.yMin, (double)(this.zMax - this.zMin) / 2.0 + (double)this.zMin);
    }
    ...
    public List<Block> edgeBlocks() {
        List<Block> edgeBlocks = new ArrayList();

        for(int x = this.xMin; x <= this.xMax; ++x) {
            for(int y = this.yMin; y <= this.yMax; ++y) {
                for(int z = this.zMin; z <= this.zMax; ++z) {
                    if (x == this.xMin || x == this.xMax || y == this.yMin || y == this.yMax || z == this.zMin || z == this.zMax) {
                        edgeBlocks.add(this.world.getBlockAt(x, y, z));
                    }
                }
            }
        }
